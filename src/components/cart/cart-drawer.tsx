...
function CartItemRow({ item, onClose }: { item: CartItem; onClose: () => void }) {
  const { updateQuantity, removeItem } = useCartStore();

  // Get the product link based on type
  const productLink =
    item.productType === "supplier"
      ? `/supplier-products/${item.productId}`
      : `/product/${item.product?.slug}`;

  // Get display info - SKU only for curated products
  const sku = item.product?.sku;

  return (
    <li className="flex gap-4 p-3 bg-secondary/30 rounded-lg">
      {/* Product image */}
        ...
        <div className="flex-1 overflow-auto p-4">
          {isEmpty ? (
            ...
            <ul className="space-y-4">
              {items.map((item) => (
                <CartItemRow key={item.productId} item={item} onClose={closeCart} />
              ))}
            </ul>
          )}
        </div>
